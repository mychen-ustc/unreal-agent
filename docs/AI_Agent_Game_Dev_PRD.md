...
| P3 多智能体 | 第 19–30 周 | 33 个 Agent + Orchestrator + 依赖 DAG 联调；**垂直切片**；评估组 + 回退闭环 | 端到端流水线跑通（提案→垂直切片→可玩关卡）；变更传播与评估触发自动生效；Demo 含完整 HUD/音频/战斗 |
| P4 实战打磨 | 第 31–42 周 | 外部生成模型接入、质量控制、评估组上线、3–4 个生物群系完整关卡 | 完整关卡由 AI 驱动生成；评估组多画像批判 + 横向对标通过；人工介入率 < 20% |
